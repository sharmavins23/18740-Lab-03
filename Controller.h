...
namespace ramulator {
    ...
    public:
        ...
        RowPolicy<T>* rowpolicy;  // determines the row-policy (e.g., closed-row
                                  // vs. open-row)
        RowTable<T>* rowtable;    // tracks metadata about rows (e.g., which are
        ...
        Queue actq;    // read and write requests for which activate was issued
                     // are moved to actq, which has higher priority than readq
                     // and writeq. This is an optimization for avoiding useless
                     // activations (i.e., PRECHARGE after ACTIVATE w/o READ of
                     // WRITE command)
        Queue otherq;  // queue for all "other" requests (e.g., refresh)

        deque<Request>
            pending;  // read requests that are about to receive data from DRAM
        bool write_mode =
            false;  // whether write requests should be prioritized over reads
        float wr_high_watermark =
            0.8f;  // threshold for switching to write mode
        float wr_low_watermark =
            0.2f;  // threshold for switching back to read mode
        ...
        Controller(const Config& configs, DRAM<T>* channel)
            : channel(channel),
              scheduler(new Scheduler<T>(configs, this)),  // Saugata
            ...
            if (record_cmd_trace) {
                ...
                string prefix = cmd_trace_prefix + "chan-" +
                                to_string(channel->id) + "-rank-";
            ...
            read_row_hits.init(configs.get_core_num())
                .name("read_row_hits_channel_" + to_string(channel->id) +
                      "_core")
                .desc(
                    "Number of row hits for read requests per channel per core")
                .precision(0);
            read_row_misses.init(configs.get_core_num())
                .name("read_row_misses_channel_" + to_string(channel->id) +
                      "_core")
                .desc(
                    "Number of row misses for read requests per channel per "
                    "core")
                .precision(0);
            read_row_conflicts.init(configs.get_core_num())
                .name("read_row_conflicts_channel_" + to_string(channel->id) +
                      "_core")
                .desc(
                    "Number of row conflicts for read requests per channel per "
                    "core")
                .precision(0);
            ...
            write_row_hits.init(configs.get_core_num())
                .name("write_row_hits_channel_" + to_string(channel->id) +
                      "_core")
                .desc(
                    "Number of row hits for write requests per channel per "
                    "core")
                .precision(0);
            write_row_misses.init(configs.get_core_num())
                .name("write_row_misses_channel_" + to_string(channel->id) +
                      "_core")
                .desc(
                    "Number of row misses for write requests per channel per "
                    "core")
                .precision(0);
            write_row_conflicts.init(configs.get_core_num())
                .name("write_row_conflicts_channel_" + to_string(channel->id) +
                      "_core")
                .desc(
                    "Number of row conflicts for write requests per channel "
                    "per core")
                .precision(0);
            ...
            useless_activates
                ...
                .desc(
                    "Number of useless activations. E.g, ACT -> PRE w/o RD or "
                    "WR")
                .precision(0);
            ...
            read_latency_sum.name("read_latency_sum_" + to_string(channel->id))
                .desc(
                    "The memory latency cycles (in memory time domain) sum for "
                    "all read requests in this channel")
                .precision(0);
            read_latency_avg.name("read_latency_avg_" + to_string(channel->id))
                .desc(
                    "The average memory latency cycles (in memory time domain) "
                    "per request for all read requests in this channel")
                .precision(6);

            req_queue_length_sum
                .name("req_queue_length_sum_" + to_string(channel->id))
                .desc(
                    "Sum of read and write queue length per memory cycle per "
                    "channel.")
                .precision(0);
            req_queue_length_avg
                .name("req_queue_length_avg_" + to_string(channel->id))
                .desc(
                    "Average of read and write queue length per memory cycle "
                    "per channel.")
                .precision(6);
            ...
            write_req_queue_length_avg
                ...
                .desc(
                    "Write queue length average per memory cycle per channel.")
                .precision(6);

#ifndef INTEGRATED_WITH_GEM5
            record_read_hits.init(configs.get_core_num())
                .name("record_read_hits")
                .desc(
                    "record read hit count for this core when it reaches "
                    "request limit or to the end");

            record_read_misses.init(configs.get_core_num())
                .name("record_read_misses")
                .desc(
                    "record_read_miss count for this core when it reaches "
                    "request limit or to the end");

            record_read_conflicts.init(configs.get_core_num())
                .name("record_read_conflicts")
                .desc(
                    "record read conflict count for this core when it reaches "
                    "request limit or to the end");

            record_write_hits.init(configs.get_core_num())
                .name("record_write_hits")
                .desc(
                    "record write hit count for this core when it reaches "
                    "request limit or to the end");

            record_write_misses.init(configs.get_core_num())
                .name("record_write_misses")
                .desc(
                    "record write miss count for this core when it reaches "
                    "request limit or to the end");

            record_write_conflicts.init(configs.get_core_num())
                .name("record_write_conflicts")
                .desc(
                    "record write conflict for this core when it reaches "
                    "request limit or to the end");
#endif
        }
        ...
        ~Controller() {
            ...
            delete channel;
            delete refresh;
            for (auto& file : cmd_trace_files) file.close();
        ...
        void finish(long read_req, long dram_cycles) {
            ...
            read_req_queue_length_avg =
                read_req_queue_length_sum.value() / dram_cycles;
            write_req_queue_length_avg =
                write_req_queue_length_sum.value() / dram_cycles;
        ...
        bool enqueue(Request& req) {
            ...
            if (queue.max == queue.size()) return false;
        ...
        void tick() {
            // * This is the cycle count tracking in the lab handout
            clk++;
            req_queue_length_sum +=
                readq.size() + writeq.size() + pending.size();
            ...
            if (pending.size()) {
                ...
                if (req.depart <= clk) {
                    if (req.depart - req.arrive >
                        1) {  // this request really accessed a row
                        read_latency_sum += req.depart - req.arrive;
                        channel->update_serving_requests(req.addr_vec.data(),
                                                         -1, clk);
                    }
                    req.callback(req);
                    pending.pop_front();
            ...
            if (!write_mode) {
                ...
                if (writeq.size() >
                        (unsigned int)(wr_high_watermark * writeq.max) ||
                    readq.size() == 0)
                    write_mode = true;
            } else {
                // no -- write queue is almost empty and read queue is not empty
                if (writeq.size() <
                        (unsigned int)(wr_low_watermark * writeq.max) &&
                    readq.size() != 0)
                    write_mode = false;
            }
            ...
            if ((req == queue->q.end() || !is_ready(req)) && actq.size() == 0) {
                ...
                if (otherq.size())
                    queue = &otherq;  // "other" requests are rare, so we give
                                      // them precedence over reads/writes

                req = scheduler->get_head(queue->q);
            }

            if (req == queue->q.end() || !is_ready(req)) {
                // we couldn't find a command to schedule -- let's try to be
                // speculative
            ...
            // only executes if this is the first time a command is being issued
            // for the selected request
            // * This is line 395 from the lab handout
            if (req->is_first_command) {
                // 18-740: This is what executes when a new request has been
                // selected for scheduling For BLISS, this is where you want to
                ...
                int tx = (channel->spec->prefetch_size *
                          channel->spec->channel_width / 8);
                if (req->type == Request::Type::READ) {
                    if (is_row_hit(req)) {
            ...
            // check whether this is the last command (which finishes the
            // request)
            // if (cmd != channel->spec->translate[int(req->type)]){
            if (!(channel->spec->is_accessing(cmd) ||
                  channel->spec->is_refreshing(cmd))) {
                if (channel->spec->is_opening(cmd)) {
                    // promote the request that caused issuing activation to
                    // actq
        ...
        // For telling whether this channel is busying in processing read or
        // write
    ...
    private:
        ...
        void cmd_issue_autoprecharge(typename T::Command& cmd,
            ...
            if (channel->spec->is_accessing(cmd) &&
                rowpolicy->type == RowPolicy<T>::Type::ClosedAP) {
                ...
                for (auto itr = queue->q.begin(); itr != queue->q.end();
                     ++itr) {
                    if (is_row_hit(itr)) {
                        auto begin2 = itr->addr_vec.begin();
                        vector<int> rowgroup2(begin2,
                                              begin2 + int(T::Level::Row) + 1);
                        if (rowgroup == rowgroup2) num_row_hits++;
                    }
                }
                ...
                if (num_row_hits == 0) {
                    ...
                    for (auto itr = queue->q.begin(); itr != queue->q.end();
                        ...
                        if (is_row_hit(itr)) {
                            auto begin2 = itr->addr_vec.begin();
                            vector<int> rowgroup2(
                                begin2, begin2 + int(T::Level::Row) + 1);
                            if (rowgroup == rowgroup2) num_row_hits++;
                        }
                    }
                }
                ...
                assert(num_row_hits >
                       0);  // The current request should be a hit,
        ...
        void issue_cmd(typename T::Command cmd, const vector<int>& addr_vec) {
            ...
            if (print_cmd_trace) {
                printf("%5s %10ld:",
                       channel->spec->command_name[int(cmd)].c_str(), clk);
                for (int lev = 0; lev < int(T::Level::MAX); lev++)
                    printf(" %5d", addr_vec[lev]);
                printf("\n");
            }
        }
        vector<int> get_addr_vec(typename T::Command cmd,
    ...
    template <>
    vector<int> Controller<SALP>::get_addr_vec(SALP::Command cmd,
                                               list<Request>::iterator req);
    ...
    template <>
    void Controller<TLDRAM>::cmd_issue_autoprecharge(
        typename TLDRAM::Command& cmd, const vector<int>& addr_vec);
