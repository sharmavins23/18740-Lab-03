...
namespace ramulator {
    ...
    protected:
        ...
        // Structure for keeping track of lines generated by core
        typedef struct {
            bool coreLine0;
            long coreLine0Addr;
            bool coreLine1;
        ...
        coreLineTracker_t coreLineTracker[4] = {{false, 0, false, 0},
                                                {false, 0, false, 0},
                                                {false, 0, false, 0},
                                                {false, 0, false, 0}};
        ...
        // For waypart: Only make new lines if we are below 2 size for the core
        std::list<Line>& get_lines_waypart(long addr, int coreID) {
            // Look for a line (if it exists)
            if (cache_lines.find(get_index(addr)) == cache_lines.end()) {
                // Ensure that we have space for the new line
                if (coreLineTracker[coreID].coreLine0 == false) {
                    // Make a new coreLine0
                    coreLineTracker[coreID].coreLine0 = true;
            ...
            // Here, we've found a line
                ...
                // Check to ensure that we own that line
                if (coreLineTracker[coreID].coreLine0Addr == addr ||
                    coreLineTracker[coreID].coreLine1Addr == addr) {
                    // We own this coreline, so we can use it
                    return cache_lines[get_index(addr)];
                } else {
                    // We don't own this line, so naively, let's return one of
                    //  our lines
                    auto& line0 = cache_lines[get_index(
                        coreLineTracker[coreID].coreLine0Addr)];
                    auto& line1 = cache_lines[get_index(
                        coreLineTracker[coreID].coreLine1Addr)];

                    // Return the least full one
            ...
            // If we already have an existing line, return it
            return cache_lines[get_index(addr)];
        }
    };
