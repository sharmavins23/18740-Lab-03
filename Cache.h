...
namespace ramulator {
    ...
    protected:
        ...
        std::list<Line>& get_lines_waypart(long addr, int coreID) {
            ...
            // Save the indexes to our core particularly
            if (coreLineTracker[coreID].coreLine0 == false) {
                coreLineTracker[coreID].coreLine0 = true;
                coreLineTracker[coreID].coreLine0Index = get_index(addr);

                return cache_lines[coreLineTracker[coreID].coreLine0Index];
            } else if (coreLineTracker[coreID].coreLine1 == false) {
                coreLineTracker[coreID].coreLine1 = true;
                coreLineTracker[coreID].coreLine1Index = get_index(addr);

                return cache_lines[coreLineTracker[coreID].coreLine1Index];
            }

            // Ensure that the line is one of our lines
            if (coreLineTracker[coreID].coreLine0Index == get_index(addr) ||
                coreLineTracker[coreID].coreLine1Index == get_index(addr)) {
                return cache_lines[get_index(addr)];
            } else {
                // It's not one of our lines, so let's arbitrarily return our
                // line
                return cache_lines[coreLineTracker[coreID].coreLine0Index];
            }
        };
